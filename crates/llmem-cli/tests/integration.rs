...
/// Get the path to the llmem binary built by cargo.
fn llmem_bin() -> PathBuf {
    let mut path = PathBuf::from(env!("CARGO_BIN_EXE_llmem"));
    // Fallback: if the macro doesn't resolve, try target/debug
    if !path.exists() {
        path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug/llmem");
    }
    path
}

/// Run llmem with the given args in an isolated HOME directory.
/// Returns (stdout as JSON, exit code).
fn run(home: &std::path::Path, args: &[&str]) -> (Value, i32) {
    let output = Command::new(llmem_bin())
        .args(args)
        .env("HOME", home)
        .output()
        .expect("failed to execute llmem");
